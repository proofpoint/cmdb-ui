...
PP.showInstanceConsole=function(sys)
{
	var p=new Ext.Panel({
		...
		autoScroll: true,
		loader:{
			url: PP.config.pcm_api_path + 'v2/clouds/' + sys.data.cloud + '/instances/' + sys.data.uuid.toLowerCase() + '/console_log',
			autoLoad: false,
			loadMask: true,
...
PP.rebootInstance=function(sys)
{
	...
	Ext.Ajax.request({
		method:'PUT',
		timeout: 520000,
		url: PP.config.pcm_api_path + 'v2/clouds/' + sys.data.cloud + '/instances/' + sys.data.uuid.toLowerCase(),
