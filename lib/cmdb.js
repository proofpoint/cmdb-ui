...
PP.rolesEditorWindow = function()
{
    ...
    w.on("close",function()
    {
        PP.reloadComboStores('roles');
    });

    w.show();
}

PP.reloadComboStores= function(name) 
{
    var components=Ext.ComponentQuery.query('combo[name=' + name + ']');
    for(i=0; i < components.length; i++)
    {
        components[i].store.load();
    }
}
