...
Ext.define('PP.SystemGrid',
{
    ...
    pendingRender: function(value, o, r, row, col, store)
    {
        ...
        {
            out = '<div class=showitem><div style="float:left"><span style="font-style:italic;font-size:10px;cursor: pointer;text-decoration:underline;color:blue;" onclick="pendingChanges(\'' + r.data.fqdn + '\');">Pending</span> ' + 
            value + '</div>'; 
        }
        else
        {
            out = '<div class=showitem><div style="float:left;">' + value + '</div>';
        }
        out += '<div class=hiddenitem><a data-qtip="ssh://' + r.data.fqdn + '" class=consoleiconlet href="ssh://' + r.data.fqdn + '">&gt;_</a></div></div>';
        return out;
    },
            ...
            },
            {
                ...
                menu:
                {
                    items: [
                        ...
                        {
                            text: 'Instance Interactive Console',
                            hidden: !PP.config.ncc_api,
                            handler: function()
                            {
                                var rec = this.getSelectionModel().getSelection()[0];
