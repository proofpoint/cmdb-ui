...
PP.graphiteWindow=function(){
    var c = Ext.getCmp('system_grid').getStore().collect('fqdn');
    var g = new PP.GraphitePanel({title: undefined});
...
Ext.define('PP.SystemGrid',{
			...
			listeners:{
				...
				edit:function(ed,e){
					if(PP.allowEditing)
					{
						...
						if(e.originalValues.status!='decommissioned' && e.newValues.status=='decommissioned')
						{
							Ext.MessageBox.show({
								title:'Warning',
								msg: 'Setting the status to decommissioned will not terminate the instance. You must select the instance and select Actions -> Terminate Selected Prism Instance',
	...
	doSystemSearch: function(){
		...
		this.store.load({params:{query:qs}});
	},
	colSizes:{
		fqdn:160,
		system_type:75,
		...
		var newtbar=new Ext.Toolbar({
			...
			items:[
				...
				},		
				{
					...
					onTriggerClick: function(){ this.triggerScope.doSystemSearch()},			
					listeners: {
    ...
    initComponent: function(){
		...
		{
			Ext.each(['fqdn','ip_address','status','roles','environment_name','svc_id','tags','notes'],function(ff){
		...
		this.counter=new Ext.toolbar.Item({text: '0'});
				...
				menu:{
					items:[
						...
						},
						{
							...
							handler: PP.graphiteWindow,
							hidden:!PP.config.enableGraphite
						},
						'-',
						{
						...
						{
							text:'Create Prism Instance',
							handler: PP.newInstanceWindow,
							hidden: !PP.config.pcm_api	
						},
						{
						...
						{
							text: 'Reboot Selected Prism Instance',
							hidden: !PP.config.pcm_api,
							handler: function(){
								var rec=this.getSelectionModel().getSelection()[0];
								if(!rec)
						...
						{
							text: 'Terminate Selected Prism Instance',
							hidden: !PP.config.pcm_api,
							handler: function(){
								var rec=this.getSelectionModel().getSelection()[0];
								if(!rec)
								...
								{
									Ext.Msg.alert('Error','The selection does not appear to be a Prism instance. (no value found for cloud or serial_number)');
									return;
								}
								if(rec.data.status=='decommissioned')
								{
		...
		this.store= new Ext.data.Store({
			...
			proxy: {
				type: 'rest',
				timeout: 20000000,
				url: PP.config.inv_api_path+'system',
		...
		this.store.on('load',function(){this.counter.update(' ' + this.store.getCount());},this);
	...
	columnConfig:function(field){
		var editorConfig=PP.getEditorConfig(field,{entity:'system',labels:false});
			...
			items:[
				{
					...
					height:18,
					border: 0,
					bodyStyle: "background-color:#dfe8f6;padding:4px;font-size:7pt;",
