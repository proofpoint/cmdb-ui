...
function inheritedTip(val, meta, rec, rowIndex, colIndex, store) {
    ...
    if(tiptext.length > 0)
    {
        meta.tdAttr = 'data-qtip="' + tiptext.replace(/"/g,'&quot;').replace(/\s/g,'&nbsp;') + '"';
    }
    return val;
};
Ext.define('PP.ServiceViewPanel',
{
    ...
    listeners: {
        'dirtychange': {
            fn: function(form,dirty,eOpts){ 
                if(dirty)
                {
                    // form.load(form.selectedService);                    
                }
            }
        }
    },
        ...
        this.grid = new Ext.grid.Panel(
        {
            ...
            plugins: (this.editing ? [
                Ext.create('Ext.grid.plugin.CellEditing',
                {
                    ...
                    {
                        scope: this,
                        fn: function(editor,event,eOpts)
                        {
                            // check for and protect against fields that are a part of the service_instance
                            val = event.value;
                            if(event.field='name')
                            {
                                if (val == 'name' ||
                                    val == 'note' ||
                                    val == 'environment_name' ||
                                    val == 'svc_id' ||
                                    val == 'value')
                                {
                                    event.cancel=true;
                                    PP.notify.msg("Error","Reserved word used. (name,note,svc_id,environment_name,value)");

                                }
                            }
                            // need to restrict single quotes
                            if(event.field = 'value')
                            {
                                if( event.value.match(/'/) )
                                {
                                    Ext.defer(editor.startEdit,250,editor,[event.record,event.column]);
                                    PP.notify.msg("Error","Single qoutes should not be used in values for service records");
                                }
                            }
                        }
                    },
