...
Ext.define('changes',{
	...
	proxy:{
		...
		url: PP.config.cmdb_api_path + 'change_queue'
	}
});
...
Ext.define('PP.ChangesGrid',{
			...
			handler:function(d){
				Ext.each(this.up('panel').getSelectionModel().getSelection(),function(i){
					...
					Ext.Ajax.request.defer(timer,Ext.Ajax,[{
						url: PP.config.cmdb_api_path + 'change_queue/' + i.id,
						method: 'DELETE',
	...
	initComponent:function(){
		...
		this.view.on('expandbody',function(rowNode, record){
			if(record.get('changes')=='loading...'){
				Ext.Ajax.request({
					url: PP.config.cmdb_api_path + 'system/' + record.data.entity_key,
					method: 'GET',
					record: record,
					row:rowNode,
					scope: this,
	...
	saveChangeRecord:function(entity,key,content,id,callback)
	{
		...
		var url=PP.config.cmdb_api_path + entity + '/' + key;
		Ext.Ajax.request({
	        url: url,
	        method: 'PUT',
	        ...
	        success: function(r, o) {
				// 
				// Ext.Ajax.request({
				// 	url: PP.config.cmdb_api_path+ 'change_queue/' + id,
				// 	grid:o.grid,
				// 	method: 'DELETE',
				// 	success: function(r,o)
				// 	{
