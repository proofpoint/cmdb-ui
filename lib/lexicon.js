...
// returns a storeconfig for an enumerator that specifies an entity + attribute as its population
PP.getEnumeratorStore = function(entity,field)
{
    if(typeof PP.entities[entity][field]._enumeration['entity'] != 'string' || typeof PP.entities[entity][field]._enumeration['attribute'] != 'string')
    {
        PP.log("ERROR: could not produce enumerator store config for " + entity + " -> " + field + "becuase there was an error with the lexicon");
        return {};
    }
    var entityField = entity + "_" + field;
    Ext.define(entityField,{
        extend: 'Ext.data.Model',
        fields: PP.getEntityFields(PP.entities[entity][field]._enumeration['entity'])
    });
    var storeconfig={
        model: (entityField),
        proxy: {
            type: 'rest',
            url: PP.config.cmdb_api_path + PP.entities[entity][field]._enumeration['entity'],
            reader: {
                type: 'json'
            },
    ...
    else
    {
        ...
        if (PP.entities[opts.entity][editor_name] != undefined && PP.entities[opts.entity][editor_name]._enumeration != undefined)
        {
            PP.editors[editor_name] = { //new Ext.form.ComboBox({
                ...
                // if enumeration specifies a field from another entity, use that
                displayField: (PP.entities[opts.entity][editor_name]._enumeration['attribute'] ? PP.entities[opts.entity][editor_name]._enumeration['attribute'] : 'value'),
                forceSelection: ( PP.entities[opts.entity][editor_name]._enumeration['forceselect'] == 'false' ? false : true),
                // get a store config based on enumeration parameters if an entity is specified, otherwise use enumerator array
                store: (PP.entities[opts.entity][editor_name]._enumeration['entity'] ? PP.getEnumeratorStore(opts.entity,editor_name) :  PP.entities[opts.entity][editor_name]._enumeration.enumerator),
                listeners:
                {
                    focus: function(f)
                    {
